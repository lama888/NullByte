...
def display_text_on_canvas(event=None):
    canvas.delete("all")
    text = text_widget.get("1.0", tk.END).strip()
    if text:
        canvas.create_text(10, 10, anchor="nw", text=text, fill=drawing_color, font=("Arial", line_width*3))
        update_status("Text displayed on canvas")
...
canvas = tk.Canvas(photo_frame, bg="white", highlightthickness=0)
canvas.pack(fill="both", expand=True)

text_frame = tk.Frame(root)
text_frame.grid(row=1, column=0, padx=10, pady=10, sticky="nsew", columnspan=2)
...
text_widget = tk.Text(text_frame, height=10, width=100, font=("Arial", 10), wrap=tk.WORD)
...
text_widget.bind("<Return>", display_text_on_canvas)
